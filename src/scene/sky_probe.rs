...
fn optical_depth(origin: Vec3, dir: Vec3, length: f32) -> (f32, f32) {
    ...
    (depth_r * step_size, depth_m * step_size)
}
...
fn sky_radiance(ray_dir: Vec3, sun_dir: Vec3, settings: &SceneSettings) -> Vec3 {
    ...
    for i in 0..NUM_SAMPLES {
        ...
        if sun_hit.1 > 0.0 {
            let (light_depth_r, light_depth_m) = optical_depth(pos, sun_dir, sun_hit.1);
    ...
    let sun_color = settings.sun_color * settings.sun_strength;
    sun_color * (total_r * BETA_R * phase_r * rayleigh_mult + total_m * BETA_M * phase_m * mie_mult)
}
