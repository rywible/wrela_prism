...
//
// Reads depth from the previous mip level and writes the max depth
// to the current mip level. Each dispatch builds one mip level.
...
fn hzb_build(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Sample 2x2 block from source and take the maximum (farthest depth)
    ...
    textureStore(dst_depth, gid.xy, vec4<f32>(max_depth, 0.0, 0.0, 0.0));
}
